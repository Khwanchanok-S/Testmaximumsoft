...
export default function Editpage() {
  ...
  const [input, setInput] = useState(dataShow);
  if (!dataShow) {
    return <div>Loading...</div>; // return some loading indicator or error message
  }

  const handleSubmitForm = async e => {
    e.preventDefault();
    try {
      const response = await axios.patch(`http://localhost:3031/users/${id}`, {
        name: input.name,
    ...
    <>
      <div className="py-4 px-8">
        ...
        <div className="w-full h-full py-20 px-80">
          <div className=" bg-orange-200  border rounded-lg w-full h-full flex  justify-center">
            <div className="w-full px-10 py-4 space-y-4">
              <div className="flex justify-center"> Edit Employees Profile</div>
